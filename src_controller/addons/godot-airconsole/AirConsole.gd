...
signal received_email_address(email_address)
signal received_profile_picture(device_id)
...
signal premium_connected(device_id)
...
func request_profile_picture(device_id_or_uid, size = 64) -> String:
	var url = js('airconsole.getProfilePicture(%s, %s)' % [to_json(device_id_or_uid), size])
	_download_profile_picture(url, device_id_or_uid)
	return url

func _download_profile_picture(url, device_id):
	$ImageRequest.request(url, device_id)

func get_profile_picture(device_id_or_uid):
	return $ImageRequest.get_texture(device_id_or_uid)
...
func _enter_tree():
	var image_request = load('res://addons/godot-airconsole/ImageRequest.gd').new()
	image_request
	image_request.name = 'ImageRequest'
	image_request.connect('received_texture', self, '_forward_image_result')
	add_child(image_request, true)

func _forward_image_result(device_id):
	emit_signal('received_profile_picture', device_id)
...
func _init():
	...
	js("airconsole = new AirConsole({ synchronize_time:true, orientation: 'landscape',device_motion:10 })")
	js("signals = []")
	...
	js("""
		...
		function get_signals() {
			var value = JSON.stringify(signals)
			signals = []
			return value
		}
	""")
...
func _setup_signals():
	...
	_setup_signal('onEmailAddress', 'received_email_address', ['email_address'])
	...
	_setup_signal('onPremium', 'premium_connected', ['device_id'])
...
func _setup_signal(airconsoleMethod:String, signalName:String, parameters:PoolStringArray):
	...
	"""airconsole.{airconsole} = function({parameters}) {
		signals.push(new Signal('{signalName}', [{parameters}]))
	}"""
...
func _poll_signals():
	...
	for s in signals:
		var arguments = s.args
		if print_signal:
			print_debug('GodotAirConsole: Signal %s(%s)' % [s.name, _to_argument_string(arguments)])
		arguments.push_front(s.name)
